...
#region SLICING_METHODS
    ...
    public static void ExplodeSprite(GameObject sprite, int numCuts, float explosionForce, int maxExplosionDepth = -1)
	{
		if(sprite)
		{
			List<SpriteSlicer2DSliceInfo> slicedObjectInfo = null;
	...
	/// <param name="slicedObjectInfo">A list of SpriteSlicer2DSliceInfo that will be fill out with details about slice locations, slcied objects, and created child objects.</param>
    /// <param name="maxExplosionDepth">How many times an individual sprite can be exploded.</param>
    public static void ExplodeSprite(GameObject sprite, int numCuts, float explosionForce, bool destroySlicedObjects, ref List<SpriteSlicer2DSliceInfo> slicedObjectInfo, int maxExplosionDepth = -1)
	{
		if(!sprite)
		{
			return;
		...
		if(GetSpriteBounds(sprite, out spriteBounds))
		{
            ...
            int explosionsSinceParentObject = 0;
            SlicedSprite parentSlicedSpriteComponent = sprite.GetComponent<SlicedSprite>();
            ...
            if(maxExplosionDepth >= 0 && explosionsSinceParentObject >= maxExplosionDepth)
            {
                return;
            }
            ...
            if(slicedObjectInfo != null)
			{
				for(int slice = 0; slice < slicedObjectInfo.Count; slice++)
				{
					...
					{
                        SlicedSprite slicedSpriteComponent = slicedObjectInfo[slice].ChildObjects[child].GetComponent<SlicedSprite>();
	...
	public static void ShatterSprite(GameObject spriteObject, float explosionForce, bool destroySlicedObjects, ref List<SpriteSlicer2DSliceInfo> slicedObjectInfo)
	{
		...
		if(polygonPoints != null && polygonPoints.Count > 3)
		{
			...
			for(int loop = 0; loop < triangles.Length; loop += 3)
			{
				...
				{
                    childSprite.InitFromUnitySprite(parentUnitySprite, ref childCollider, ref childSpriteVertices, false, parentUnitySprite.color);
				}
					...
					if(polygonPoints != null)
					{
						...
						if(IsConvex(ref polygonPoints))
						{
							...
							else
							{
								...
								{
                                    childSprite1.InitFromUnitySprite(parentUnitySprite, ref child1Collider, ref childSprite1Vertices, false, spriteColor);
                                    childSprite2.InitFromUnitySprite(parentUnitySprite, ref child2Collider, ref childSprite2Vertices, false, spriteColor);
								}
						...
						else // Use concave slicing method
						{
							...
							for(int childIndex = 0; childIndex < s_ConcaveSlicePolygonResults.Count; childIndex++)
							{
								...
								if(AreVerticesAcceptable(ref childSpriteVertices, childArea, false))
								{
									...
									{
                                        childSprite.InitFromUnitySprite(parentUnitySprite, ref childCollider, ref childSpriteVertices, true, spriteColor);
									}
...
public class SlicedSprite : MonoBehaviour
{
    ...
    public int ExplosionsSinceParentObject { get { return m_ExplosionsSinceParentObject; } set { m_ExplosionsSinceParentObject = value; } }
    ...
    int m_ExplosionsSinceParentObject;
    bool m_Rotated;
    bool m_VFlipped;
    bool m_HFlipped;
    ...
    public void InitFromSlicedSprite(SlicedSprite slicedSprite, ref PolygonCollider2D polygon, ref Vector2[] polygonPoints, bool isConcave)
    {
        ...
        InitSprite(slicedSprite.gameObject, slicedSprite.MeshRenderer, ref polygon, ref polygonPoints, slicedSprite.MinCoords, slicedSprite.MaxCoords, slicedSprite.SpriteBounds, slicedSprite.m_MeshRenderer.sharedMaterial, slicedSprite.Rotated, slicedSprite.HFlipped, slicedSprite.VFlipped, slicedSprite.m_Centroid, slicedSprite.m_UVOffset, isConcave, slicedSprite.m_Color);
    ...
    /// </summary>
    public void InitFromUnitySprite(SpriteRenderer unitySprite, ref PolygonCollider2D polygon, ref Vector2[] polygonPoints, bool isConcave, Color32 spriteColor)
    {
        ...
        InitSprite(unitySprite.gameObject, unitySprite.GetComponent<Renderer>(), ref polygon, ref polygonPoints, minTextureCoords, maxTextureCoords, unitySprite.sprite.bounds, material, false, false, false, Vector2.zero, pivotVector, isConcave, spriteColor);
    ...
    /// </summary>
    void InitSprite(GameObject parentObject, Renderer parentRenderer, ref PolygonCollider2D polygon, ref Vector2[] polygonPoints, Vector3 minCoords, Vector3 maxCoords, Bounds spriteBounds, Material material, bool rotated, bool hFlipped, bool vFlipped, Vector2 parentCentroid, Vector2 uvOffset, bool isConcave, Color32 spriteColor)
    {
