...
namespace Core.Logic
{
    ...
    public class QuestEvaluator
    {
        ...
        public QuestResult EvaluateQuest(QuestData quest, PathNetworkState network)
        {
            ...
            for (int i = 0; i < quest.EntitiesToConnect.Count; i++)
            {
        ...
        {
            int totalRequirements = quest.EntitiesToConnect.Count + quest.PathsToDisconnect.Count;
            ...
            int groupIndex)
        {
            if (group.OnlyAClump)
            {
                return (groupIndex, true, "Clump group (no connection requirement)");
            }
            
            if (group.EntityIds.Count == 0)
            {
                return (groupIndex, false, "Empty group");
            }
            
            if (group.EntityIds.Count == 1)
            {
                return (groupIndex, true, "Single entity (automatically satisfied)");
            }
            ...
            int firstPathId = network.GetPathId(pathPoints[0]);
            ...
            PathNetworkState network)
        {
            ...
            int pathPoint1 = GetRepresentativePathPoint(group1);
            int pathPoint2 = GetRepresentativePathPoint(group2);
        ...
        public QuestStatus GetDetailedStatus(QuestData quest, PathNetworkState network)
        {
            ...
            foreach (var disconnectReq in quest.PathsToDisconnect)
            {
                var group1 = quest.EntitiesToConnect[disconnectReq.GroupIndex1];
                var group2 = quest.EntitiesToConnect[disconnectReq.GroupIndex2];
                bool satisfied = CheckGroupsDisconnected(group1, group2, network);
